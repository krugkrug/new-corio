...
```bash
git clone https://github.com/krugkrug/alf-alf-whatsapp-mcp-server.git
cd alf-whatsapp-mcp-server
npm install
npm run build
```
...
```json
{
  "mcpServers": {
    "whatsapp": {
      "command": "node",
      "args": ["/path/to/alf-whatsapp-mcp-server/dist/mcp-server/index.js"],
      "env": {
